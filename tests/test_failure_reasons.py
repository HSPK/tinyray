...
def test_last_error_is_empty_while_things_work_and_fills_in_when_they_stop(registry):
    """silence_ms 说失联了，last_error 说失联长什么样。"""
    with tinyray.join("c", "churn") as me:
        me.ready()
        assert me.last_error == "", me.last_error
        registry.stop()
        deadline = time.monotonic() + 15
        while not me.last_error and time.monotonic() < deadline:
            time.sleep(0.05)
        assert me.last_error, "注册中心死了 15s，一条原因都没记下"
        assert me.silence_ms > 0
