...
class TestProcessTrees:
    """Stopping a process must stop everything it started.

    `torchrun` spawns one child per GPU; SGLang spawns schedulers and workers.
    Signalling only the parent leaves those children alive, holding GPU memory
    that nothing will reclaim -- and every restart strands another set.
    """
    ...
    SPAWNER = textwrap.dedent(
        ...
        kids = [subprocess.Popen([sys.executable, "-c", "import time; time.sleep(300)"])
                for _ in range(3)]
        print("CHILDREN " + ",".join(str(k.pid) for k in kids), flush=True)
        time.sleep(300)
        """
    )
    ...
    def _children_of(self, process) -> list[int]:
        log = "".join(process.recent_log())
        line = log.split("CHILDREN ")[1].split("\n")[0]
        return [int(pid) for pid in line.split(",")]

    def test_stopping_a_parent_stops_its_children(self, ray):
        process = tinyray.launch_process(
            [PYTHON, "-c", self.SPAWNER],
            name="tree",
            ready_when="log:CHILDREN",
            allocate_port=False,
            startup_timeout=60,
        )
        children = self._children_of(process)
        assert all(self._alive(pid) for pid in children)

        tinyray.stop_process("tree")
        time.sleep(1.5)

        survivors = [pid for pid in children if self._alive(pid)]
        # Clean up before asserting, so a failure does not leak processes too.
        import os
        ...
        assert not survivors, (
            f"{len(survivors)} orphaned children survived; a torchrun job or an "
            "inference server would keep its GPUs after every restart"
        )

    def test_shutdown_stops_whole_trees(self):
        tinyray.shutdown()
        tinyray.init()
        process = tinyray.launch_process(
            [PYTHON, "-c", self.SPAWNER],
        ...
        tinyray.shutdown()
        time.sleep(1.5)
        assert not [pid for pid in children if self._alive(pid)]


class TestProcessRestart:
    """A supervised process is supervised, not merely started."""

    CRASHER = (
        "import time, sys, os; print('up', os.getpid(), flush=True); time.sleep(1.0); sys.exit(42)"
    )

    def test_a_dead_process_is_restarted_within_its_budget(self, ray):
        process = tinyray.launch_process(
            [PYTHON, "-c", self.CRASHER],
            name="restarting",
            num_cpus=0.5,
            ready_when="log:up",
            allocate_port=False,
        ...
        deadline = time.monotonic() + 20
        while time.monotonic() < deadline:
            current = {p.name: p for p in tinyray.processes()}.get("restarting")
            if current is not None and current.pid != first_pid:
                assert current.restarts >= 1
                break
            time.sleep(0.2)
        else:
            pytest.fail("the process was never restarted")

    def test_the_budget_is_finite_and_resources_come_back(self, ray):
        before = tinyray.nodes()[0]["available_cpus"]
        tinyray.launch_process(
            [PYTHON, "-c", self.CRASHER],
            name="doomed-eventually",
            num_cpus=1.0,
            ready_when="log:up",
            allocate_port=False,
            max_restarts=1,
        ...
        while time.monotonic() < deadline:
            if "doomed-eventually" not in [p.name for p in tinyray.processes()]:
                break
            time.sleep(0.2)
        else:
            pytest.fail("the process was restarted past its budget")

        # A server that is gone for good must not keep holding its GPUs.
        assert tinyray.nodes()[0]["available_cpus"] == before

    def test_without_a_budget_a_dead_process_is_reaped(self, ray):
        before = tinyray.nodes()[0]["available_cpus"]
        tinyray.launch_process(
            [PYTHON, "-c", "import sys; sys.exit(1)"],
            name="no-budget",
        ...
        deadline = time.monotonic() + 20
        while time.monotonic() < deadline:
            if "no-budget" not in [p.name for p in tinyray.processes()]:
                break
            time.sleep(0.2)
        else:
            pytest.fail("a dead process stayed in the registry, still holding resources")
        assert tinyray.nodes()[0]["available_cpus"] == before

    def test_repeated_restarts_do_not_invent_resources(self, ray):
        """Releasing a reservation twice does not lose a GPU -- it creates one.

        The scheduler then oversubscribes hardware that does not exist, and the
        symptom appears much later as two processes on one device.
        """
        total = tinyray.nodes()[0]["total_cpus"]
        before = tinyray.nodes()[0]["available_cpus"]
        ...
        while time.monotonic() < deadline:
            if "churn" not in [p.name for p in tinyray.processes()]:
                break
            time.sleep(0.2)

        after = tinyray.nodes()[0]["available_cpus"]
        assert after == before, f"available CPUs drifted from {before} to {after}"
        assert after <= total, f"the scheduler now believes in {after} of {total} CPUs"
